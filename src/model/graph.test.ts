...
import { Graph, buildGraph, edgeIdFor, emptyGraph, flipMate } from './graph';
import { link, mate } from './testing';
...
const nodesOnly = (...ids: BrickId[]): Graph =>
  emptyGraph().addBricks(ids.map((brick) => ({ brick })));

describe('edge identity', () => {
  it('is orientation independent', () => {
    expect(edgeIdFor('b1', 'b2')).toBe(edgeIdFor('b2', 'b1'));
  });
});
...
describe('addBrick', () => {
  it('adds an isolated node with empty adjacency', () => {
    const g = emptyGraph().addBrick('b1');
    expect(g.nodes.get('b1')).toEqual({ brick: 'b1', out: [], in: [], peer: [] });
  ...
  it('creates one edge carrying every mate for a brick pair', () => {
    ...
    const g = nodesOnly('b1').addBrick('b2', [
  ...
  it('places a gendered edge in out for the male side and in for the female side', () => {
    const g = nodesOnly('b1').addBrick('b2', [link('b1', 'b2', [mate('s1', 'k1', 'a')])]);
    const id = edgeIdFor('b1', 'b2');
    expect(g.nodes.get('b1')?.out).toEqual([id]);
    expect(g.nodes.get('b1')?.in).toEqual([]);
    expect(g.nodes.get('b2')?.in).toEqual([id]);
    expect(g.nodes.get('b2')?.out).toEqual([]);
  });

  it('places a symmetric edge in peer on both sides', () => {
    const g = nodesOnly('h1').addBrick('h2', [
      link('h1', 'h2', [mate('f1', 'f2', 'symmetric', 'finger')]),
    ]);
    const id = edgeIdFor('h1', 'h2');
    expect(g.nodes.get('h1')?.peer).toEqual([id]);
    expect(g.nodes.get('h2')?.peer).toEqual([id]);
    expect(g.nodes.get('h1')?.out).toEqual([]);
    expect(g.nodes.get('h2')?.in).toEqual([]);
    expectConsistent(g);
  });
  ...
  it('classifies an edge with both gendered directions as peer', () => {
    ...
    const id = edgeIdFor('b1', 'b2');
    expect(g.nodes.get('b1')?.peer).toEqual([id]);
    expect(g.nodes.get('b2')?.peer).toEqual([id]);
    expectConsistent(g);
  });

  it('merges a second link on the same pair into the existing edge', () => {
    const g = nodesOnly('b1')
      .addBrick('b2', [link('b1', 'b2', [mate('s1', 'k1')])])
  ...
  it('flips mates supplied in the opposite orientation when merging', () => {
    const g = nodesOnly('b1')
      .addBrick('b2', [link('b1', 'b2', [mate('s1', 'k1', 'a')])])
  ...
  it('rejects a link to an unknown brick, a self link, and a link it is not part of', () => {
    expect(() =>
      nodesOnly().addBrick('b1', [link('b1', 'ghost', [mate('s', 'k')])]),
    ).toThrow(/unknown brick/);
    expect(() => nodesOnly('b1').addBrick('b2', [link('b2', 'b2', [mate('s', 'k')])])).toThrow();
    expect(() =>
      nodesOnly('b1', 'b3').addBrick('b2', [link('b1', 'b3', [mate('s', 'k')])]),
...
describe('removeBrick', () => {
  it('cleans adjacency in both directions', () => {
    const g = nodesOnly('b1')
      .addBrick('b2', [link('b1', 'b2', [mate('s1', 'k1')])])
      .removeBrick('b2');
    expect(g.nodes.has('b2')).toBe(false);
    expect(g.edges.size).toBe(0);
    expect(g.nodes.get('b1')).toEqual({ brick: 'b1', out: [], in: [], peer: [] });
    expectConsistent(g);
  });
  ...
  it('ignores an unknown brick', () => {
    const g = nodesOnly('b1');
    expect(snapshot(g.removeBrick('ghost'))).toEqual(snapshot(g));
  });
...
describe('neighbors and component', () => {
  ...
  it('reports each neighbour once regardless of direction', () => {
    expect([...chain().neighbors('b')].sort()).toEqual(['a', 'c']);
    expect(chain().neighbors('lonely')).toEqual([]);
    expect(chain().neighbors('ghost')).toEqual([]);
  });

  it('traverses out, in and peer alike', () => {
    expect([...chain().component('a')].sort()).toEqual(['a', 'b', 'c', 'd']);
    expect([...chain().component('d')].sort()).toEqual(['a', 'b', 'c', 'd']);
    expect([...chain().component('lonely')]).toEqual(['lonely']);
    expect([...chain().component('ghost')]).toEqual([]);
  });

  it('splits the component when the joining brick is removed', () => {
    const split = chain().removeBrick('b');
    expect([...split.component('a')]).toEqual(['a']);
    expect([...split.component('c')].sort()).toEqual(['c', 'd']);
  ...
  it('reports degree and incident edges', () => {
    ...
    expect(g.edgesOf('b').map((e) => e.id).sort()).toEqual(
...
describe('buildGraph', () => {
  it('builds nodes and edges from a solved edge list', () => {
    const g = buildGraph(['b1', 'b2', 'b3'], [
      { id: 'ignored', a: 'b1', b: 'b2', mates: [mate('s1', 'k1'), mate('s2', 'k2')] },
    ]);
    expect(g.nodes.size).toBe(3);
    expect(g.edges.size).toBe(1);
    // Edge ids are canonicalised from the brick pair.
    expect(g.edges.has(edgeIdFor('b1', 'b2'))).toBe(true);
    expectConsistent(g);
  });

  it('rejects an edge referencing an unknown brick', () => {
    expect(() =>
      buildGraph(['b1'], [{ id: 'e', a: 'b1', b: 'ghost', mates: [mate('s', 'k')] }]),
    ).toThrow(/unknown brick/);
  });
});
...
describe('structural sharing', () => {
  it('reuses node objects for untouched bricks', () => {
    ...
    const next = base.addBrick('b4', [link('b1', 'b4', [mate('s', 'k')])]);
    expect(next.nodes.get('b2')).toBe(base.nodes.get('b2'));
    expect(next.nodes.get('b3')).toBe(base.nodes.get('b3'));
